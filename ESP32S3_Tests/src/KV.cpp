...
void onRunKVTest(char* cmd) {
    // Parse custom voltage if provided (e.g., sending "K4" tests at 4V)
    float testV = atof(cmd);
    if (testV <= 0.0f) testV = 3.0f; // Safe default

    Serial.println("\n=== RUNNING SIMPLEFOC AUTO-CHARACTERIZATION ===");
    Serial.printf("Testing with %.1f Volts. Please wait...\n", testV);

    // 1. HALT Core 0! (Prevents dual-core SPI/PWM collisions)
    pauseFOCForSPI = true; 
    delay(10); // Give Core 0 a millisecond to safely exit its current FOC loop

    // 2. Run the built-in library function!
    // This blocks for a few seconds and prints R, L, and KV directly to Serial.
    motor.characteriseMotor(testV);

    // 3. Return control to Core 0
    pauseFOCForSPI = false; 
