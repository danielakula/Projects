...
BLDCMotor      motor  = BLDCMotor(14, 0.5f, 310.0f, 0.00018f);
...
uint32_t kvTestStartTime = 0;
float testVoltage = 1.5f;

void onRunKVTest(char* cmd) {
    if (!kvTestRunning) {
        float requestedV = atof(cmd);
        if (requestedV > 0.0f && requestedV <= 5.0f) {
            testVoltage = requestedV;
        } else {
            testVoltage = 3.0f; // Safe default
        }
        
        // 1. Switch controller to open-loop voltage
        ...
        kvTestRunning = true;
        kvTestStartTime = millis();
        
        Serial.println("\n=== STARTING MANUAL KV TEST ===");
        Serial.printf("Applying %.2f Volts. Waiting 3 seconds for steady state...\n", testVoltage);
    }
}
...
void setup() {
    ...
    command.add('K', onRunKVTest, "Run Auto-Characterization");
    ...
    float R = 0.5;           
    float L = 0.00018;
    float bandwidth_hz = 150.0; // loopFOC() ~10kHz, L*2pi*bandwidth=R, reduced to ensure stability
...
void loop() {
    ...
    if (millis() - last > 25) {
        ...
        // ================================================================
        //  CUSTOM KV TEST STATE MACHINE
        // ================================================================
        if (kvTestRunning) {
            // Wait exactly 3000ms for the wheel to reach terminal velocity
            if (millis() - kvTestStartTime > 3000) {
                
                // 1. Grab steady-state telemetry from Core 0
                float speed_rads = motor.shaft_velocity;
                float rpm = speed_rads * (30.0f / PI);
                float current_q = motor.current.q;
                
                // 2. Calculate True Back-EMF
                // V_bemf = Applied_Voltage - (Measured_Current * Phase_Resistance)
                float phase_R = 0.537f; // Your hardcoded motor resistance
                float v_bemf = testVoltage - (current_q * phase_R);
                
                // 3. Calculate KV Rating
                float raw_kv = rpm / testVoltage;
                float true_kv = rpm / v_bemf;
                
                // 4. Safely shut down and revert to FOC Current mode
                target = 0.0f;
                motor.torque_controller = TorqueControlType::foc_current;
                kvTestRunning = false;
                ...
                // 5. Print the results
                Serial.println("\n=== KV TEST RESULTS ===");
                Serial.printf("Applied Voltage : %.2f V\n", testVoltage);
                Serial.printf("Measured Iq     : %.2f A\n", current_q);
                Serial.printf("Back-EMF (est)  : %.2f V\n", v_bemf);
                Serial.printf("Steady Speed    : %.2f rad/s (%.0f RPM)\n", speed_rads, rpm);
                Serial.printf("Raw KV (V_tgt)  : %.0f RPM/V\n", raw_kv);
                Serial.printf("True KV (BEMF)  : %.0f RPM/V\n", true_kv);
                Serial.println("=======================\n");
            }
        }
